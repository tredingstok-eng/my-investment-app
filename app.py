...
IB_QUERY = "1489351"
# הוספת משתנה רנדומלי ל-URL כדי למנוע Cache של Google
...
# הפונקציה עכשיו מקבלת timestamp כדי להכריח רענון
@st.cache_data(ttl=10) # רענון כל 10 שניות במקום דקה
def load_data(ts):
    try:
        # הוספת הפרמטר z ל-URL מבטיחה שגוגל ישלח נתונים טריים
        fresh_url = f"{SHEET_URL}&z={ts}"
...
# קריאה לנתונים עם הזמן הנוכחי כ"מפתח" לרענון
df, total_nav = load_data(int(time.time()))
...
if not st.session_state.auth:
    ...
    # כפתור התנתקות קטן וצדדי
    with st.sidebar:
        if st.button("Logout 🚪"):
            st.session_state.auth = False
            st.rerun()
        st.write(f"עודכן לאחרונה: {datetime.now().strftime('%H:%M:%S')}")
